...
researcher は **Streamlit ベースの WebUI** を提供しています。以下の機能をサポート：
...
1. **チャット**: テキストボックスに質問を入力してEnterキーを押します
2. **自動検索**: クエリに基づいて自動的にSearXNGで検索します
3. **セッション管理**: サイドバーから既存セッションを選択、新規作成、検索、削除が可能です
4. **モデル選択**: サイドバーのドロップダウンからOllamaモデルを切り替えます
5. **言語切り替え**: 日本語/英語を切り替え、セッションに保存されます

詳細なガイドは [📘 Streamlit WebUI ガイド](docs/streamlit-guide.md) をご覧ください。

⚠️ **セキュリティに関する重要な注意**: Streamlit WebUI はローカル個人使用を想定しています。認証機構がなく、セッションデータは暗号化されていません。詳細は [🔒 セキュリティガイド](docs/security.md) をご確認ください。
