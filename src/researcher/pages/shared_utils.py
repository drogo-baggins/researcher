...
def initialize_session_chat():
    ...
    if is_first_init:
        ...
        try:
            ...
            for model_key in ["search_model", "response_model", "eval_model"]:
                model_name = settings.get(model_key)
                if model_name and model_name not in available_models:
                    st.warning(f"⚠️ 設定されたモデル '{model_name}' ({model_key}) が見つかりません")
        except Exception as e:
    ...
    agent = QueryAgent(search_ollama_client, language=language) if searxng_client else None
    
    # Create ChatManager
    if "chat_manager" not in st.session_state:
        chat_manager = ChatManager(
            ollama_client=response_ollama_client,
            ...
            searxng_engine=settings.get("searxng_engine"),
            searxng_lang=settings.get("searxng_lang"),
            searxng_safesearch=settings.get("searxng_safesearch"),
        )
    ...
    if "model" not in st.session_state:
        st.session_state.model = response_model
